...
class Model {
    ...
    public function save($data='',$options=array()) {
        if(empty($data)) {
            ...
            if(!empty($this->data)) {
                ...
                $this->error    =   L('_DATA_TYPE_INVALID_');
                // \Think\Log::write('1 ====: '.$this->error);
                return false;
            }
        }
        // 数据处理
        // \Think\Log::write('origin ====: '.dump($data,false));
        $data       =   $this->_facade($data);
        if(empty($data)){
            // 没有数据则不执行
            $this->error    =   L('_DATA_TYPE_INVALID_');
            // \Think\Log::write('new ====: '.dump($data,false));
            // \Think\Log::write('2 ====: '.$this->error);
            return false;
        }
        // 分析表达式
        $options    =   $this->_parseOptions($options);
     ...
     */
     public function create($data='',$type='') {
        ...
        // log_("Think\Model::create()-1.data",$data,$this);/////////////////////

        // thinkphp:如果没有传值默认取POST数据，被我改了，为空的话就返回false
        if(empty($data)) {
            $this->error = -407;
            return false;
        ...
        if(empty($data) || !is_array($data)) {
            ...
            // log_("Think\Model::create()-2.data",$data,$this);
            // log_("Think\Model::create()-empty($data)",empty($data),$this);
            // log_("Think\Model::create()-is_array($data)",is_array($data),$this);
        ...
        {
            log_("Think\Model::create()-autoValidation.data",$data,$this);/////////////////////
            return false;
        }
        ...
        // 返回创建的数据以供其他调用
        // log_("Think\Model::create()-last.data",$data,$this);/////////////////////
        return $data;
     }
    ...
    protected function autoValidation($data,$type) {
        // log_("\Think\Model::autoValidation()-1.data",$data,$this);//////////////////
        // log_("\Think\Model::autoValidation()-1.type",$type,$this);//////////////////
        ...
        if(isset($_validate)) { // 如果设置了数据自动验证则进行数据验证
                ...
                if(empty($val[5]) || ( $val[5]== self::MODEL_BOTH && $type < 3 ) || $val[5]== $type ) {
                    ...
                    switch($val[3]) {
                        case self::MUST_VALIDATE:   // 必须验证 不管表单是否有设置该字段
                            ...
                            {
                                // log_("MUST_VALIDATE",null,$this);///////////////////
                                // log_("data",$data,$this);////////////////////
                                // log_("val",$val,$this);///////////////////////
                                // log_("data[$val[0]]",$data[$val[0]],$this);/////////////////
                                // log_("this->error",$this->error,$this);///////////////
                                return false;
                            }
                            break;
                        ...
                        default:    // 默认表单存在该字段就验证
                            if(isset($data[$val[0]]))
                            {
                                // log_("默认表单存在该字段就验证",null,$this);///////////////////
                                // log_("默认表单存在该字段就验证-data",$data,$this);////////////////////
                                // log_("默认表单存在该字段就验证-val",$val,$this);///////////////////////
                                // log_("默认表单存在该字段就验证-data[$val[0]]",$data[$val[0]],$this);/////////////////
                                // log_("默认表单存在该字段就验证-this->error",$this->error,$this);///////////////
                                // log_("this->_validationField($data,$val)",$this->_validationField($data,$val),$this);
                                if(false === $this->_validationField($data,$val)) 
                                    return false;
                            }
                    }
            ...
            if(!empty($this->error))
            {
                log_("批量验证的时候最后返回错误",null,$this);///////////////////
                log_("批量验证的时候最后返回错误-this->error",$this->error,$this);///////////////
                return false;
            }
        }
        return true;
        ...
        switch(strtolower(trim($val[4]))) {
            ...
            case 'callback':// 调用方法进行验证
                ...
                if('function'==$val[4]) {
                    ...
                    // \Think\Log::write('======>val[1]<br>'.dump($val[1],false));
                    // \Think\Log::write('======>this<br>'.dump($this,false));
     ...
     * @return void
     */
    public function startTrans($methodName = null)
    {
        $this->commit($methodName.'---startTrans之前的自动commit');
        $this->db->startTrans($methodName);
        return ;
    }
     ...
     * @return boolean
     */
    public function commit($methodName = null)
    {
        return $this->db->commit($methodName);
    }
     ...
     * @return boolean
     */
    public function rollback($methodName = null)
    {
        return $this->db->rollback($methodName);
    }
    ...
    public function data($data=''){
        ...
        if(is_object($data)){
            ...
            log_("Think\Model::data()-data",$data,$this);
